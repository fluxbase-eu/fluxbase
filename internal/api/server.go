...
func NewServer(cfg *config.Config, db *database.Connection, version string) *Server {
	...
	functionsHandler := functions.NewHandler(db, cfg.Functions.FunctionsDir, cfg.CORS, cfg.Auth.JWTSecret, functionsInternalURL, cfg.Functions.NpmRegistry, authService, loggingService, secretsStorage)
	...
	if cfg.Jobs.Enabled {
		...
		var err error
		jobsHandler, err = jobs.NewHandler(db, &cfg.Jobs, jobsManager, authService, loggingService, cfg.Functions.NpmRegistry)
		if err != nil {
