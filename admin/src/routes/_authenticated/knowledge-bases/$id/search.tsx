...
function KnowledgeBaseSearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      toast.success(
        `Found ${response.results?.length || 0} results (${response.mode} search)`
      )
    } catch {
        ...
        <CardContent>
          <div className='grid gap-4'>
            ...
            <p className='text-muted-foreground text-xs'>
              <strong>Hybrid:</strong> Combines semantic meaning + keyword
              matching. <strong>Semantic:</strong> Finds conceptually similar
              content. <strong>Keyword:</strong> Finds exact text matches.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {debugResult && (
        <Card className='border-amber-500/50 bg-amber-500/5'>
          <CardHeader>
            ...
            <CardDescription>
              Raw similarity scores and embedding details (no threshold
              filtering)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className='grid gap-4 font-mono text-sm'>
              ...
              <div className='grid grid-cols-3 gap-4'>
                <div>
                  ...
                  <span className='font-semibold'>
                    {debugResult.total_chunks}
                  </span>
                </div>
                <div>
                  <span className='text-muted-foreground'>With Embedding:</span>{' '}
                  <span
                    className={`font-semibold ${debugResult.chunks_with_embedding === 0 ? 'text-red-500' : 'text-green-500'}`}
                  >
                ...
                <div>
                  ...
                  <span
                    className={`font-semibold ${debugResult.chunks_without_embedding > 0 ? 'text-red-500' : ''}`}
                  >
              ...
              <div className='grid grid-cols-2 gap-4'>
                ...
                <div>
                  ...
                  <span
                    className={`font-semibold ${debugResult.embedding_model !== debugResult.kb_embedding_model ? 'text-amber-500' : ''}`}
                  >
              ...
              <div>
                <span className='text-muted-foreground'>
                  Query Embedding Dims:
                </span>{' '}
                <span className='font-semibold'>
                  {debugResult.query_embedding_dims}
                </span>
              </div>
              <div>
                <span className='text-muted-foreground'>
                  Query Embedding Preview (first 10):
                </span>
                <pre className='mt-1 overflow-x-auto rounded bg-black/20 p-2 text-xs'>
                  [
                  {debugResult.query_embedding_preview
                    .map((v) => v.toFixed(6))
                    .join(', ')}
                  ]
                </pre>
              ...
              {debugResult.stored_embedding_preview && (
                <div>
                  <span className='text-muted-foreground'>
                    Stored Embedding Preview (first 10):
                  </span>
                  <pre className='mt-1 overflow-x-auto rounded bg-black/20 p-2 text-xs'>
                    [
                    {debugResult.stored_embedding_preview
              ...
              <div>
                ...
                {debugResult.raw_similarities.length === 0 ? (
                  <span className='ml-2 text-red-500'>
                    No results - all chunks may have NULL embeddings
                  </span>
                ) : (
                  <pre className='mt-1 overflow-x-auto rounded bg-black/20 p-2 text-xs'>
                    [
                    {debugResult.raw_similarities
                      .map((v) => v.toFixed(6))
                      .join(', ')}
                    ]
                  </pre>
      ...
      >
        <DialogContent className='max-h-[80vh] max-w-2xl'>
          ...
          <ScrollArea className='max-h-[60vh]'>
            <p className='pr-4 text-sm whitespace-pre-wrap'>
