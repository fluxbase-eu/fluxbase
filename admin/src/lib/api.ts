...
// Lazy-loaded branch store to avoid circular dependency issues
// The branch store imports branchesApi from this file, so we can't import it at the top level
/* eslint-disable @typescript-eslint/no-require-imports */
let _branchStoreModule: typeof import("@/stores/branch-store") | undefined;
const getBranchStore = () => {
  if (!_branchStoreModule) {
    _branchStoreModule = require("@/stores/branch-store");
  }
  return _branchStoreModule!.useBranchStore;
};
...
api.interceptors.request.use(
  (config) => {
    ...
    // Add X-Fluxbase-Branch header if a non-main branch is selected
    try {
      const currentBranch = getBranchStore().getState().currentBranch;
      // Only add header if we have a non-main branch selected
      if (currentBranch?.slug && currentBranch.type !== "main") {
        config.headers["X-Fluxbase-Branch"] = currentBranch.slug;
      }
    } catch {
      // Branch store not available, skip header
    }
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // Skip token refresh for endpoints that may return 401 for non-auth reasons
    // e.g., /api/v1/admin/branches returns 401 when branching is disabled (route not registered)
