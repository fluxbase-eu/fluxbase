 ...
 */
export function createMockClient(overrides: Partial<FluxbaseClient> = {}): FluxbaseClient {
  return {
    auth: {
      ...
      updateUser: vi.fn().mockResolvedValue({ id: '1', email: 'test@example.com' }),
    ...
    storage: {
      from: vi.fn().mockReturnValue({
        ...
        move: vi.fn().mockResolvedValue({ data: { path: 'new.txt' }, error: null }),
        copy: vi.fn().mockResolvedValue({ data: { path: 'copy.txt' }, error: null }),
      }),
    ...
    admin: {
      ...
      resetUserPassword: vi.fn().mockResolvedValue({ data: { message: 'Password reset' }, error: null }),
      settings: {
        app: {
 ...
 */
export function createWrapper(client: FluxbaseClient, queryClient?: QueryClient) {
...
export function renderWithProviders(
  ...
  const { client = createMockClient(), queryClient, ...renderOptions } = options || {};
