...
export class ServiceKeysManager {
   ...
   * ```
   */
  async get(id: string): Promise<{ data: ServiceKey | null; error: Error | null }> {
    try {
  ...
  async create(
    ...
    try {
      const data = await this.fetch.post<ServiceKeyWithKey>('/service-keys', request);
      return { data, error: null };
    } catch (error) {
      return { data: null, error: error as Error };
    }
  ...
  async update(
    ...
    try {
      const data = await this.fetch.put<ServiceKey>(`/service-keys/${id}`, request);
      return { data, error: null };
    } catch (error) {
      return { data: null, error: error as Error };
    }
  ...
  async deprecate(
    ...
    try {
      ...
      const data = await this.fetch.post<{ deprecated_at: string; grace_period_ends_at: string }>(
        `/service-keys/${id}/deprecate`,
        body,
      );
      return { data, error: null };
    } catch (error) {
   ...
   * ```
   */
  async rotate(id: string): Promise<{ data: ServiceKeyWithKey | null; error: Error | null }> {
    try {
      const data = await this.fetch.post<ServiceKeyWithKey>(`/service-keys/${id}/rotate`, {});
      return { data, error: null };
    } catch (error) {
      return { data: null, error: error as Error };
    }
  ...
  async getRevocationHistory(
    id: string,
  ): Promise<{ data: { id: string; name: string; revoked_at: string; revoked_by: string; revocation_reason: string } | null; error: Error | null }> {
    try {
