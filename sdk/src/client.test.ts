...
vi.mock('./fetch', () => ({
  FluxbaseFetch: vi.fn().mockImplementation(() => ({
    setAnonKey: vi.fn(),
    setAuthToken: vi.fn(),
    get: vi.fn(),
    post: vi.fn(),
...
vi.mock('./auth', () => ({
  FluxbaseAuth: vi.fn().mockImplementation(() => ({
...
vi.mock('./realtime', () => ({
  FluxbaseRealtime: vi.fn().mockImplementation(() => ({
    setAuth: vi.fn(),
...
vi.mock('./storage', () => ({
  FluxbaseStorage: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./functions', () => ({
  FluxbaseFunctions: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./jobs', () => ({
  FluxbaseJobs: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./admin', () => ({
  FluxbaseAdmin: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./management', () => ({
  FluxbaseManagement: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./settings', () => ({
  SettingsClient: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./ai', () => ({
  FluxbaseAI: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./vector', () => ({
  FluxbaseVector: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./graphql', () => ({
  FluxbaseGraphQL: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./branching', () => ({
  FluxbaseBranching: vi.fn().mockImplementation(() => ({})),
}))

vi.mock('./rpc', () => ({
  FluxbaseRPC: vi.fn().mockImplementation(() => ({
    invoke: vi.fn().mockResolvedValue({ data: { result: 'test' }, error: null }),
    list: vi.fn(),
    getStatus: vi.fn(),
    getLogs: vi.fn(),
...
vi.mock('./schema-query-builder', () => ({
  SchemaQueryBuilder: vi.fn().mockImplementation(() => ({
...
describe('createClient', () => {
  ...
  it('should support generic database type', () => {
    ...
    const client = createClient<MyDatabase>('http://localhost:8080', 'test-key')
    expect(client).toBeInstanceOf(FluxbaseClient)
  })
})
